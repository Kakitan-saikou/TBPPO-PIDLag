...
class Buffer:
    ...
    def sample_offline(self, batch_size=256, length=1):
        if batch_size > self.size:
            batch_size = self.size - 1
        ind = np.random.randint(0, self.size, size=batch_size)
        ind2 = np.array([np.random.randint(0, self.size1, size=length) for _ in range(batch_size)])

        state = self.offline_buffer_dict['states'][ind[:, None], ind2, :, :]
        action = self.offline_buffer_dict['actions'][ind[:, None], ind2, :]
        reward = self.offline_buffer_dict['rewards'][ind[:, None], ind2]
        done = self.offline_buffer_dict['dones'][ind[:, None], ind2]
        state_next = self.offline_buffer_dict['states_next'][ind[:, None], ind2, :, :]
        sa_pair = self.offline_buffer_dict['states_actions'][ind[:, None], ind2, :, :]
        fx = self.offline_buffer_dict['fx'][ind[:, None], ind2]
        fy = self.offline_buffer_dict['fy'][ind[:, None], ind2]
        advs = self.offline_buffer_dict['advantages'][ind[:, None], ind2]

        return state, action, sa_pair, reward, done, state_next, fx, fy, advs

    def load_gae_adv(self, advs):
        self.offline_buffer_dict['advantages'] = list()
        self.offline_buffer_dict['advantages'] = advs
        print("Advantages pre-computed by gae loaded")
    ...
    def load_all_offlines(self, path, path2, gamma, l=119, clip=None):
        #S, S', SA, A, R, D, Rt
        ...
        for item in offline_properties:
            ...
            if item != 'returns':
                s = [arr[:l] for arr in data[item]]
                s = np.concatenate(s, axis=1)
                if path2 != None:
                    s2 = [arr[:l] for arr in data2[item]]
                    s2 = np.concatenate(s2, axis=1)
                    s = np.concatenate((s, s2), axis=1)
                    print("Two datasets concated, ", str(item),  " new shape: ", s.shape)

            if 'state' in item:

                if item == 'states_actions':
                    self.offline_buffer_dict[item] = np.array(s).transpose(1, 0, 2, 3) / 12.0
                    print("SA_pairs scaled!")
                else:
                    self.offline_buffer_dict[item] = np.array(s).transpose(1, 0, 2, 3)
            elif 'rewards' in item or 'dones' in item or 'fx' in item or 'fy' in item:
                self.offline_buffer_dict[item] = np.array(s).transpose(1, 0)
                # print(item)
            elif item == 'actions':
                if clip != None:
                    self.offline_buffer_dict[item] = np.clip(np.array(s), -clip, clip).transpose(1, 0, 2)
                    print("All actions clipped to {}, {}".format(-clip, clip))
                else:
                    self.offline_buffer_dict[item] = np.array(s).transpose(1, 0, 2)

        self.cal_return(gamma)
        self.size = self.offline_buffer_dict['states'].shape[0]
        self.size1 = self.offline_buffer_dict['states'].shape[1]
        ...
        fx = self.offline_buffer_dict['fx']
        fy = self.offline_buffer_dict['fy']

        # print(type(self.offline_buffer_dict['fx']), type(self.offline_buffer_dict['rewards']))

        return state, state_next, sa_pair, action, reward, done, returns, fx, fy
