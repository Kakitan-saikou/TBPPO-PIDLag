...
class EnsembleLinearDecoder(nn.Module):
    def __init__(self, n_ensemble, in_dim, hid_dim, out_dim): 
        super().__init__()
        self.w1 = torch.randn(n_ensemble, in_dim, hid_dim).cuda()
        self.b1 = torch.randn(n_ensemble, 1, hid_dim).cuda()
        self.w2 = torch.randn(n_ensemble, hid_dim, out_dim).cuda()
        self.b2 = torch.randn(n_ensemble, 1, out_dim).cuda()
    ...
    def forward(self, in_feature):
        if len(in_feature.shape) == 2:
            ...
            # 3-dim input, each ensembled critic process each ensembled actor
            hidden = self.swish_func((torch.einsum('nbi,nid->nbd', in_feature, self.w1) + self.b1))
            out_feature = torch.einsum('nbd,ndo->nbo', hidden, self.w2) + self.b2
...
class TransformerQAgent(nn.Module):
    def __init__(
        ...
        self.encoder = Transformer(self.encoder_dim,
                                   ...
                                   self.encoder_head,
                                   decoder_dim_head,
                                   self.encoder_dim * 4).to(self.device) # TODO:dropout, dim_head
        ...
        self.to_obs_embed = nn.Sequential(
            nn.Linear(self.obs_dim, self.embed_dim).to(self.device)
        )
        self.to_action_embed = nn.Sequential(
            nn.Linear(self.action_dim, self.embed_dim).to(self.device)
        )
        ...
        self.decoder1 = EnsembleLinearDecoder(self.n_ensemble, self.embed_dim * 2, self.embed_dim, 1).to(self.device)
        self.decoder2 = EnsembleLinearDecoder(self.n_ensemble, self.embed_dim * 2, self.embed_dim, 1).to(self.device)
    ...
    def get_encoded_obs(self, obs):
        ...
        obs_pos_embedding = repeat(self.obs_pos_embedding, 'b n d -> (b b_repeat) n d', b_repeat=batch_num).to(self.device)
        ...
        first_o_token = encoded_tokens[:, -1]

        return first_o_token
    ...
    def forward(self, obs=None, action=None, train=False):
        ...
        if len(action.shape) == 4:
            #Process ensembled situations
            # action = rearrange(action, 'n b d -> (n b) d')
            # act_token = self.to_action_embed(action)
            # n-> n_ensemble, b n_iff, t timestep, e embeddim, c combined (ib)
            act_token = rearrange(act_token, 'n b t e -> n (b t) e')
            first_o_token = self.get_encoded_obs(obs)
            first_o_tokens = repeat(first_o_token, 'c e -> n c e', n=self.n_ensemble)
            first_token = torch.cat((first_o_tokens, act_token), dim=-1)
        else:
            #No ensemble
            # act_token = self.to_action_embed(action)
            act_token = rearrange(act_token, 'b t e -> (b t) e')
            first_o_token = self.get_encoded_obs(obs)
            first_token = torch.cat((first_o_token, act_token), dim=-1)

        q1, q2 = self.decoder1(first_token).squeeze(-1), self.decoder2(first_token).squeeze(-1)
        
        if re_flag == True:
            q1 = rearrange(q1, 'n (b t) -> n b t', b=b)
            q2 = rearrange(q2, 'n (b t) -> n b t', b=b)
        
        return q1, q2

    def get_q1(self, obs, action_list):
        ...
        act_token = self.to_action_embed(action_list)

        if len(action_list.shape) == 4:
            #Process ensembled situations
            # action = rearrange(action, 'n b d -> (n b) d')
            # act_token = self.to_action_embed(action)
            # n-> n_ensemble, i n_iff, b batch, e embeddim, c combined (ib)
            act_token = rearrange(act_token, 'n i b e -> n (i b) e')
            first_o_token = self.get_encoded_obs(obs)
            first_o_tokens = repeat(first_o_token, 'c e -> n c e', n=self.n_ensemble)
            first_token = torch.cat((first_o_tokens, act_token), dim=-1)
        else:
            # act_token = self.to_action_embed(action)
            act_token = rearrange(act_token, 'i b e -> (i b) e')
            first_o_token = self.get_encoded_obs(obs)
            first_token = torch.cat((first_o_token, act_token), dim=-1)

        q1 = self.decoder1(first_token).squeeze(-1)
    ...
    def getValue(self, obs, action):
        ...
        if len(obs.shape) == 4:
            ...
            b = obs.shape[0]
            obs = rearrange(obs, "b t c d-> (b t) c d")
            action = rearrange(action, "b t a-> (b t) a")
        ...
        if re_flag == True:
            value1 = rearrange(value1, "(b t) 1 -> b t 1", b=b)
            value2 = rearrange(value2, "(b t) 1 -> b t 1", b=b)
        return value1, value2

    def getExpiles(self, obs, action_list):
        #Only for inferencing, now action -> n_ensembl, n_iff, embed_dim
        first_o_token = self.get_encoded_obs(obs)
        first_o_tokens = repeat(first_o_token, 'i e -> n i e', n=self.n_ensemble)

        act_token = self.to_action_embed(action_list)

        first_token = torch.cat((first_o_tokens, act_token), dim=-1)

        q1 = self.decoder1.forward(first_token)
        q2 = self.decoder2.forward(first_token)

        # n_ensemble, n_iff ->  n_iff, n_ensemble
        q = torch.min(q1, q2).squeeze(-1)
        q_list = rearrange(q, 'n i -> i n')

        return q_list



    def getEnsembleValue(self, obs, action_list):
        value_list1, value_list2 = self.forward(obs, action_list)
...
class TransformerAgent(nn.Module):
    def __init__(
        ...
        self.n_agent = config.context_len # self.n_agent means the max input block
        self.n_ensemble = config.n_ensemble
        self.action_dim = config.action_space # since mean and std are concatenated
        ...
        self.decoder = EnsembleLinearDecoder(self.n_ensemble, self.embed_dim, self.embed_dim, self.action_dim).to(self.device)
        # learn parameters
        self.optimizer = self.configure_optimizers()
    ...
    def forward(self, obs=None, train=False):
        ...
        action_list = self.decoder(first_token)
        # action_list = []
        # for i in range(self.n_ensemble):
    ...
    def getEnsembleAction(self, obs, train=True, Vector=False):
        re_flag = False
        if len(obs.shape) == 4:
            re_flag = True
            b = obs.shape[0]
        ...
        if re_flag == True:
            a_ = rearrange(a_, 'n (b t) d -> n b t d', b=b)
        
        if Vector == True:
            action_list = a_
        else:
            action_list = a_.detach().cpu().numpy()
