...
class TransformerQAgent(nn.Module):
    ...
    def forward(self, obs=None, action=None, train=False):
        ...
        action_list1 = self.decoder1(first_token)
        action_list2 = self.decoder2(first_token)
    ...
    def get_q1(self, obs, action):
        ...
        first_o_token = self.get_encoded_obs(obs)
        first_token = torch.cat((first_o_token, act_token), dim=1)
        value1 = self.decoder1(first_token)
        if re_flag == True:
    ...
    def getValue(self, obs, action):
        ...
        if re_flag == True:
            value1 = rearrange(value1, "(b t) 1 -> b t 1", b=b)
            value21 = rearrange(value2, "(b t) 1 -> b t 1", b=b)
    ...
    def getExpiles(self, obs, action_list):
        ...
        q_list = [[],[],[],[],[],[],[],[]]
        for i in range(self.n_ensemble):
            act_token = self.to_action_embed(action_list[i])
            first_token = torch.cat((first_o_token, act_token), dim=1)

            q1 = self.decoder_list1[i].forward(first_token)
            q2 = self.decoder_list2[i].forward(first_token)

            q = torch.min(q1, q2).squeeze(-1)
            # 8 * 1 -> 8

            for j in range(self.n_iff):
                q_list[j].append(q[j])
                # 8 * n_ensemble
    ...
    def getEnsembleValue(self, obs, action):
        ...
        if re_flag == True:
            for i in range(self.n_ensemble):
                value_list1.append(rearrange(value1[i], "(b t) 1 -> b t 1", b=b))
                value_list2.append(rearrange(value1[i], "(b t) 1 -> b t 1", b=b))
